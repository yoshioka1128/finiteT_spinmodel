subroutine freeeng_exch_fromangle(imax,nr,free,exchloc,theta,phi,zmagS,g,SO,zmoment,zmomentS,&
...
!      free(ir)=free(ir)+eigen(1)-temp*dlog(part(ir))
   ...
   do m=1,imax
      ...
      do jj=1,imax
         do ii=1,imax
            zabc=zabc+conjg(zunit(ii,m,ir))*2.0d0*zmagS(ii,jj)*HexT(ir)*zunit(jj,m,ir)
         end do
      end do
      if(temp.eq.0.0d0) then
...
end subroutine freeeng_exch_fromangle
