!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine MF_uniform_Nd2Fe14B(nfe,nr,nfe0,nr0,nff,nrf,nrr,J1,gr,nunit,temp,magFe,magR,JRFe,JFeR,JFeFe,JRR,&
     Hex,Hex0,magR0,magFe0,magRT,magFeT,HexT,dconv)
implicit none
...
real(8) :: nff,nrr,nrf,gr,dconv
real(8) :: Zff,Zrf,Zrr,HexT(700)
real(8) :: alphaf,betar,Jr,Jf
real(8) :: JRFe,JFeR,JFeFe,JRR,Brillouin,Langevin,muFe,mu,Hex0
real(8) :: HRT,HFeT,xR,xFe,abc,def,ghi,magFe0,magR0,magFeT,magRT,J1,magFe1,magR1,magFe2,magR2,x
...
!nff=5650d0
!nrf=(-(Hex0*kb)*2.0d0*(gr-1.0d0)/gr)/(dble(nfe0)*magFe0*muB*dconv) ! input Hex(T=0)=260 K, Hex(T=290)=249 K
!nrr=0.0d0
...
! Interaction
JRFe=dble(nfe0)*dconv*muB/Zrf*gf*(gr/(gr-1.0d0))*nrf ! dconv*muB (kOe*muB=Merg) 
JFeFe=dble(nfe0)/2.0d0*dconv*muB/Zff*gf**2*nff
JRR=dconv*muB/Zrr*(gr/(gr-1.0d0))**2*nrr
...
end subroutine MF_uniform_Nd2Fe14B
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine MF_uniform2(nfe,nr,nunit,temp,magFe,magR,JRFe,JFeR,JFeFe,JRR,Hex,Hex0,magR0,magFe0,magRT,magFeT,HexT)
implicit none
...
real(8),parameter :: joule=10.0d0 ! (Merg/J)
real(8),parameter :: muB=9.274d-24 ! (Merg/kOe) or (kerg/Oe)
real(8),parameter :: dconv=4.0d0*1.0d24/(ax**2*cx)*muB ! from f.u. to density (kOe) or (kerg/Oe/cm^3)
real(8),parameter :: pi=dacos(-1.0d0)
real(8) :: temp,temp0,gf=2.0d0,gs=2.0d0,magR(nr*nunit),magFe(nfe*nunit),Hex
...
real(8) :: alphaf,betar,Jr,Jf
real(8) :: JRFe,JFeR,JFeFe,JRR,Brillouin,Langevin,muFe,mu,Hex0
real(8) :: HRT,HFeT,xR,xFe,abc,def,ghi,magFe0,magR0,magFeT,magRT,S1,L1,J1,magFe1,magR1,magFe2,magR2,x
...
gr=1.5d0+(S1*(S1+1.0d0)-L1*(L1+1.0d0))/(2.0d0*J1*(J1+1.0d0)) ! rande=8.0d0/11.0d0 for Nd
write(6,*) atom,gr,J1
...
! NdFeB molecular field coefficients by fitting from magnetization
nff=5650d0
nrf=(-Hex0*2.0d0*(gr-1.0d0)/gr*kb/muB)/14.0d0/magFe0/dconv ! input Hex(T=0)=260 K, Hex(T=290)=249 K
nrr=0.0d0
!nrr=100.0d0
...
!      write(6,"(I5,3F15.5)") ite,2.0d0*magR1,14.0d0*magFe1,magFe1*14.0d0+magR1*2.0d0
      ...
      HRT=dconv*(2.0d0*nrr*magR1+14.0d0*nrf*magFe1)
      HFeT=dconv*(14.0d0*nff*magFe1+2.0d0*nrf*magR1)
...
!   write(6,"(I5,3F15.5)") ite,2.0d0*magR1,14.0d0*magFe1,magFe1*14.0d0+magR1*2.0d0
   ...
   write(11,"(F10.3,4F25.17)") temp0,&
        ...
        dconv*(14.0d0*nrf*magFe1)*muB/kb*gr/(gr-1.0d0)/2.0d0
   HexT(itemp)=Hex0*magFe1/magFe0
!   write(6,*) itemp,HexT(itemp)
 !  write(6,*) "T, 2*muR, 14*muFe, mutot (mu_B/f.u.)"
!   write(6,"(4F15.5)") temp0,2.0d0*magR1,14.0d0*magFe1,magFe1*14.0d0+magR1*2.0d0
...
! Interaction
JRFe=14.0d0*dconv*muB/Zrf*gf*(gr/(gr-1.0d0))*nrf ! dconv*muB (kOe*muB=Merg) 
JFeFe=7.0d0*dconv*muB/Zff*gf**2*nff
JRR=dconv*muB/Zrr*(gr/(gr-1.0d0))**2*nrr
