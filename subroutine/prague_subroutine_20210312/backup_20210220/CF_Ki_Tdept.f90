subroutine CF_KiFe_Tdept2(ax,cx,Hex,temp0,Klcr,Klcfe,nr,nfe,nunit,irsub,KuFe,HexT,dK1,dK2,dK3,dK1Fe,dK2Fe,dK3Fe)
implicit none
...
real(8) :: K11,K22,K1,K2,K3,K4,K5,K11loc(4),K22loc(4),K1loc(4),K2loc(4),K3loc(4),K4loc(4),K5loc(4),Kloc(9,4),HexT(700)
...
integer(4) :: m,n,i,k,l,imax,ii,jj,INFO,LWORK,it,ip,itnum=400,ll,mm,ir,ir0,inddy,idim(15),itemp,ife
...
real(8) :: dK1(7,nr*nunit),dK2(7,nr*nunit),dK3(7,nr*nunit),dK01(7,100),dK02(7,100),dK03(7,100),dK1Fe(7,nfe*nunit),dK2Fe(7,nfe*nunit),dK3Fe(7,nfe*nunit)
...
end subroutine CF_KiFe_Tdept2




subroutine CF_Ki_Tdept2(ax,cx,Hex,temp0,Klcr,Klcfe,nr,nfe,nunit,irsub,KuFe,HexT,dK1,dK2,dK3,dK1Fe,dK2Fe,dK3Fe)
implicit none
real(8),parameter :: joule=10.0d0 ! (Merg/J)
integer(4) :: nr,nfe,nunit
real(8) :: ax,cx,eng0(4)
...
! n=4
   ...
   open(101,file="K1_"//trim(adjustl(rare))//".txt") !  dK1(1,ir) Kloc(1,ir)
   open(102,file="K2_"//trim(adjustl(rare))//".txt") !  dK2(1,ir) Kloc(2,ir)
   open(103,file="K3_"//trim(adjustl(rare))//".txt") !  dK2(5,ir) Kloc(3,ir)
   open(104,file="K11_"//trim(adjustl(rare))//".txt") ! dK1(2,ir) Kloc(6,ir)
   open(105,file="K22_"//trim(adjustl(rare))//".txt") ! dK2(2,ir) Kloc(7,ir)
...
!      write(6,*) it,ip
      ...
      do ir=1,4 ! sum up inequivalent R site
         ...
         do jj=1,imax
            do ii=1,imax
               zmatrix(ii,jj)=zmatrix(ii,jj)+2.0d0*(g(inddy)-1.0d0)*HexT(itemp)*zmag(ii,jj)
            end do
         end do
...
dK03=0.0d0
do ir=1,4
   dK01(1,ir)=K1tilloc(0,ir) ! sin**2(theta) Kloc(1) -> dK1(1)
   dK01(2,ir)=K1tilloc(2,ir)-dK01(1,ir) ! sin**2(theta)*sin(2phi) Kloc(6) -> dK1(2)
   dK02(2,ir)=(dsqrt(2.0d0)+1.0d0)*(2.0d0*K2tilloc(1,ir)-K2tilloc(0,ir)-K2tilloc(2,ir)) ! sin**4(theta)*sin(2phi) Kloc(7) -> dK2(2)
   dK02(1,ir)=K2tilloc(1,ir)-dK02(2,ir)/dsqrt(2.0d0) ! sin**4(theta) Kloc(2) -> dK2(1)
   dK02(5,ir)=K2tilloc(0,ir)-dK02(1,ir) ! sin**4(theta)*cos(4phi) Kloc(3) -> dK2(5)
end do
...
!   Ku=(K1loc(1)+K1loc(2)+K1loc(3)+K1loc(4))/4.0d0
   ...
   do ir=1,nr*nunit
      do ik=1,7
         do ir0=1,2
            if(irsub(ir).eq.ir0) dK1(ik,ir)=dK01(ik,1)
            if(irsub(ir).eq.ir0) dK2(ik,ir)=dK02(ik,1)
            if(irsub(ir).eq.ir0) dK3(ik,ir)=dK03(ik,1)
         end do
         do ir0=5,6
            if(irsub(ir).eq.ir0) dK1(ik,ir)=dK01(ik,1)
            if(irsub(ir).eq.ir0) dK2(ik,ir)=dK02(ik,1)
         ...
         do ir0=3,4
            if(irsub(ir).eq.ir0) dK1(ik,ir)=dK01(ik,4)
            if(irsub(ir).eq.ir0) dK2(ik,ir)=dK02(ik,4)
            if(irsub(ir).eq.ir0) dK3(ik,ir)=dK03(ik,4)
         end do
         do ir0=7,8
            if(irsub(ir).eq.ir0) dK1(ik,ir)=dK01(ik,4)
            if(irsub(ir).eq.ir0) dK2(ik,ir)=dK02(ik,4)
   ...
   do it=1,int(pi/Dtheta/2.0d0)
      ...
      do ir=1,4
         freeapp(ir)=(dK01(1,ir)+dK01(2,ir)*dsin(2.0d0*phi))*dsin(theta)**2+&
              (dK02(1,ir)+dK02(2,ir)*dsin(2.0d0*phi)+dK02(3,ir)*dcos(4.0d0*phi))*dsin(theta)**4
      end do
...
write(101,*) temp,dK01(1,1),dK01(1,2),(dK01(1,1)+dK01(1,2)+dK01(1,3)+dK01(1,4))*volume ! Kelvin,Kelvin,MJ/m^3
write(102,*) temp,dK02(1,1),dK02(1,2),(dK02(1,1)+dK02(1,2)+dK02(1,3)+dK02(1,4))*volume
write(103,*) temp,dK02(3,1),dK02(3,2),(dK02(3,1)+dK02(3,2)+dK02(3,3)+dK02(3,4))*volume
write(104,*) temp,dK01(2,1),dK01(2,2),(dK01(2,1)+dK01(2,2)+dK01(2,3)+dK01(2,4))*volume
write(105,*) temp,dK02(2,1),dK02(2,2),(dK02(2,1)+dK02(2,2)+dK02(2,3)+dK02(2,4))*volume
...
end subroutine CF_Ki_Tdept2



subroutine CF_Ki_Tdept(ax,cx,Hex,temp0,Klcr,Klcfe,nr,nfe,nunit,irsub,KuFe,HexT)
implicit none
real(8),parameter :: joule=10.0d0 ! (Merg/J)
integer(4) :: nr,nfe,nunit
real(8) :: ax,cx,eng0(4)
