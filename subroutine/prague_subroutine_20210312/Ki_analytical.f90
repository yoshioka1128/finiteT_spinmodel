subroutine Ki_analytical(sys,method,wwoK1Fe,Tmin,Tmax,dT,volume,lambda,Alm,rdch,K1FeT,HexT,mFeT,Tc,ratom,nr,dmult)
implicit none
...
real(8) :: abc,def,temp,S1,L1,J1,Hmax,Tmin,Tmax,dT,Femag,Tc,Ms0,g1,dcoff,KuFe,volume,dmult,Dex(nrmax),Dso,xfp
real(8) :: JM(100,2),Stevens(6),xi(6),cclm(6,-6:6),lambda
...
real(8) :: dKi1st(9),dKimix1(9),dKimix21(9),dKi2nd(9),dKidisp(9),K1FeT(itempmax)
...
character(50) :: sys,ratom,method,wwoK1Fe,rdch
...
call numfactor(cclm)
write(6,*) "Stevens factor and Xi factor check "
do ll=2,6,2
...
write(35,*) "# T [K],K1 [K/atom], K2 [K/atom], K3 [K/atom]"
write(47,*) "# T [K],K1 [K/atom], K2 [K/atom], K3 [K/atom]"
write(45,*) "# T [K],K1 [K/atom], K2 [K/atom], K3 [K/atom]"
...
open(55,file=""//trim(sys)//"_"//trim(method)//"_"//trim(wwoK1Fe)//"_K1K2K3tot_1st_mix2_rd"//trim(rdch)//".txt")
open(57,file=""//trim(sys)//"_"//trim(method)//"_"//trim(wwoK1Fe)//"_K1K2tot_2nd_mix2_rd"//trim(rdch)//".txt") 
...
do itemp=0,int((Tmax-Tmin)/dT)  ! loop for Temperature 
   ...
   do j=1,3 ! Kj 
      do ir=1,nr ! inequivalent atom 
         ...
         do l=2,6,2 ! for Alm(l)
            ! Ki w/o mixing 
            dKi1st(j)=dKi1st(j) +coffKi(j,l,0)*Stevens(l)*Alm(l,0,ir)*extBJ(l,ir) ! w/o mixing
            dclKi(j)=dclKi(j)   +coffKi(j,l,0)*Stevens(l)*Alm(l,0,ir)*extLJ(l,ir)
            ! Ki w/ J mixing from Hex*Hcf
            abc=xi(l)*dble(l*(l+1))/(dble(2*l+1))*Alm(l,0,ir)
            def=(Dex(ir)/Dso)*abc
            dKimix1(j)  =dKimix1(j)   +def*coffKi(j,l,0)*Tmix1(l,ir) ! Kuz'min (2002) and Magnani (2003) w/ mixing
            dKimix21(j) =dKimix21(j)  +def*coffKi(j,l,0)*Tmix2(l,ir)
            dclKimix1(j)=dclKimix1(j) +def*coffKi(j,l,0)*Tclmix1(l,ir)
            dclKimix2(j)=dclKimix2(j) +def*coffKi(j,l,0)*Tclmix2(l,ir)
            ! magnetic moment w/ J mixing from Hex*Hcf
            def=2.0d0*S1/(J1+1.0d0)*(1.0d0/Dso)*abc
            dmagmixcf1(1,ir)=dmagmixcf1(1,ir)       +def*coffmag(l,0)*Tmix1(l,ir)
            dmagmixcf2(1,ir)=dmagmixcf2(1,ir)       +def*coffmag(l,0)*Tmix2(l,ir)
            dmagmixcf1(2,ir)=dmagmixcf1(2,ir) -0.5d0*def*coffmag(l,0)*Tmix1(l,ir)
            dmagmixcf2(2,ir)=dmagmixcf2(2,ir) -0.5d0*def*coffmag(l,0)*Tmix2(l,ir)
         end do ! do i 
   ...
   write(35,"(4f25.15)") temp,dKi1st(1)/dble(nr),dKi1st(2)/dble(nr),dKi1st(3)/dble(nr)
   write(47,"(4f25.15)") temp,&
       (dKi1st(1)+dKimix1(1))/dble(nr),(dKi1st(2)+dKimix1(2))/dble(nr),(dKi1st(3)+dKimix1(3))/dble(nr)
   write(45,"(4f25.15)") temp,&
       (dKi1st(1)+dKimix1(1)+dKimix21(1))/dble(nr),&
       (dKi1st(2)+dKimix1(2)+dKimix21(2))/dble(nr),&
       (dKi1st(3)+dKimix1(3)+dKimix21(3))/dble(nr)! K1,K2 [MJ/m^3]
   ...
   write(43,*) temp,(dKi1st(1)+dKi2nd(1)+dKidisp(1)+dKimix1(1)+dKimix21(1))/dble(nr),&
        (dKi1st(2)+dKi2nd(2)+dKidisp(2)+dKimix1(2)+dKimix21(2))/dble(nr)
    ! classical K1
   ...
   write(55,"(4f25.15)") temp,&
       ((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff,&
       (dKi1st(2)+dKimix1(2)+dKimix21(2))*dmult*dcoff,&
       (dKi1st(3)+dKimix1(3)+dKimix21(3))*dmult*dcoff ! K1,K2 [MJ/m^3]

   write(57,*) temp,&
        ((dKi1st(1)+dKimix1(1)+dKimix21(1)+dKi2nd(1)+dKidisp(1))*dmult+KuFe)*dcoff,&
        (dKi1st(2)+dKimix1(2)+dKimix21(2)+dKi2nd(2)+dKidisp(2))*dmult*dcoff ! K1,K2 [MJ/m^3]
   ...
   Ms0=((dmag1st(1,ir)+dmagmixex1(1,ir)+dmagmixex2(1,ir)+dmag1st(2,ir)+dmagmixex1(2,ir)+dmagmixex2(2,ir))&
        *dble(nr)+Femag)*dmult*muB/volume
   write(54,*) temp,Ms0*mu0 ! magnetization [T]
   
   ! Nucleation field, FP field, MA fiel
   abc=(dKi1st(2)+dKimix1(2)+dKimix21(2))*dmult/((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)
   xfp=(-1.0d0+dsqrt(-3.0d0/abc-2.0d0))/3.0d0
   if(temp.le.Tc) then
      write(56,"(5f12.6)") temp,&
           2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff/Ms0*1.0d6,& ! Nucleation field
           2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff/Ms0*1.0d6*&
           (xfp+2.0d0*abc*xfp**3),& ! FP field  
           (2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)&
           +4.0d0*(dKi1st(2)+dKimix1(2)+dKimix21(2))*dmult)*dcoff/Ms0*1.0d6,& ! MA field
           abc*6.0d0
      if(6.0d0*abc.le.-1.0d0) then
         write(156,"(5f12.6)") temp,&
           2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff/Ms0*1.0d6*(xfp+2.0d0*abc*xfp**3),& ! FP field
           (2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)&
           +4.0d0*(dKi1st(2)+dKimix1(2)+dKimix21(2)))*dmult*dcoff/Ms0*1.0d6,& ! MA field
           2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff/Ms0*1.0d6,& ! Nucleation field
           abc*6.0d0
   ...
   write(6,*) "Ms [T]",Ms0*mu0
   write(6,*) "K1 [K]",(dKi1st(1)+dKimix1(1)+dKimix21(1))/dble(nr)
   write(6,*) "K1 [MJ/m^3]",((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)&
        *dcoff,(dKi1st(1)+dKidisp(1)+dKimix1(1)+dKimix21(1))*dmult*dcoff
   write(6,*) "HN [T]",2.0d0*((dKi1st(1)+dKimix1(1)+dKimix21(1))*dmult+KuFe)*dcoff/Ms0*1.0d6
