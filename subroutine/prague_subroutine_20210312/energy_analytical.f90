subroutine energy_analytical(temp,sys,method,volume,lambda,Alm,KuFe,Hex,ratom,nr,dmult,rdch)
implicit none
integer(4),parameter :: nrmax=30,nJmax=21
...
real(8) :: temp,S1,L1,J1,theta,phi,dcoff,Dso,Dex,abc,KuFe,g1,volume,dmult,dphi,lambda
real(8) :: JM(100,2),Stevens(6),extBJ(-1:8,nrmax),rdcoff(0:6),Alm(6,-6:6,nrmax)
real(8) :: dmagRT0(3),Tmix1(6,nrmax),Tmix2(6,nrmax),free0(3,nrmax),freeR(3,nrmax),freeFe,gibbs(3),Hex(nrmax)
...
abc=(Dex/Dso)*(2.0d0*(L1+1.0d0))/(3.0d0*(J1+1.0d0))
dmagRT0=0.0d0
do ir=1,nr
...
do it=0,itmax,itmax
   ...
   do ip=0,ipmax*2
      ...
      do ir=1,nr
         do l=2,6,2
            do m=-l,l
               ...
               abc=xi(l)*dble(l*(l+1))/(dble(2*l+1))*Dex/Dso*coffmag(l,m)*Alm(l,m,ir)
      ...
      do i=1,3
         ...
         gibbs(i)=gibbs(i)+abc*dmult+freeFe
      end do
