!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine Kuzmin(atom,ax,cx,d,nfe,nr,nunit,temp,magFe,magR,JRFe,JFeR,JFeFe,JRR,Hex,Hex0,magR0,magFe0,magRT,magFeT,HexT)
implicit none
!real(8),parameter :: ax=8.497d0,cx=4.687d0 ! (angstrom)
...
!real(8),parameter :: d=2.0d0*1.0d24/(ax**2*cx)*muB ! from f.u. to density (kOe) or (kerg/Oe/cm^3)
real(8),parameter :: pi=dacos(-1.0d0)
real(8) :: temp,temp0,gf=2.0d0,gs=2.0d0,magR(nr*nunit),magFe(nfe*nunit),Hex,ax,cx,d
real(8) :: nff,nrr,nrf,gr
real(8) :: Zff,Zrf,Zrr,HexT(4,100)
real(8) :: alphaf,betar,Jr,Jf,Tc,Ms,MsT,s
real(8) :: JRFe,JFeR,JFeFe,JRR,Brillouin,Langevin,muFe,mu,Hex0(2)
...
if(atom.eq."Nd") then
   ...
   Ltot=6.0d0
   Jtot=dabs(Ltot-Stot)
else if(atom.eq."SmFe12".or.atom.eq."SmFe12_2".or.atom.eq."SmFeCoa".or.atom.eq."SmFeCob".or.atom.eq."SmFeCoc".or.&
     atom.eq."SmFeCoa2".or.atom.eq."SmFeCob2".or.atom.eq."SmFeCoc2".or.atom.eq."Sm2Fe14B") then
   Stot=5.0d0/2.0d0
   Ltot=5.0d0
   Jtot=dabs(Ltot-Stot)
end if
...
open(11,file="magnetization_Hex_Tdept_"//trim(adjustl(atom))//".txt")
do itemp=0,100
   temp0=dble(itemp)*10.0d0
   magR1=magR0
   magFe1=magFe0
...
!      write(6,"(I5,3F15.5)") ite,2.0d0*magR1,14.0d0*magFe1,magFe1*14.0d0+magR1*2.0d0
      ...
      HRT=d*(1.0d0*nrr*magR1+12.0d0*nrf*magFe1)
      HFeT=d*(12.0d0*nff*magFe1+1.0d0*nrf*magR1)
...
!        magR1/(ax**2*cx)*muB*4.0d0*pi/10.0d0,& ! mag (muB per f.u. CGS)
!        12.0d0*magFe1/(ax**2*cx)*1.0d24*2.0d0*muB*4.0d0*pi/10.0d0,&
!        (magFe1*12.0d0+magR1)/(ax**2*cx)*1.0d24*2.0d0*muB*4.0d0*pi/10.0d0,& ! (T)
!        d*(12.0d0*nrf*magFe1)*muB/kb*gr/(gr-1.0d0)/2.0d0
   if (atom.eq."SmFeCoa".or.atom.eq."SmFeCob".or.atom.eq."SmFeCoc".or.atom.eq."SmFeCoa2".or.atom.eq."SmFeCob2".or.atom.eq."SmFeCoc2") then
      Tc=859.0d0
      s=0.73d0
      Ms=1.88d0
      MsT=Ms*(1.0d0-s*(temp0/Tc)**(1.5d0)-(1.0d0-s)*(temp0/Tc)**(2.5d0))**(1.0d0/3.0d0) ! p=5/2
      if(MsT.lt.0.0d0) MsT=0.0d0
      write(11,"(F10.3,4F25.17)") temp0,MsT
   else if(atom.eq."SmFe12".or.atom.eq."SmFe12_2") then
      Tc=555.0d0
      s=1.01d0
      Ms=1.94d0
      MsT=Ms*(1.0d0-s*(temp0/Tc)**(1.5d0)-(1.0d0-s)*(temp0/Tc)**(2.5d0))**(1.0d0/3.0d0) ! p=5/2
      if(MsT.lt.0.0d0) MsT=0.0d0
      write(11,"(F10.3,4F25.17)") temp0,MsT
   else if(atom.eq."Sm2Fe14B") then
      Ms=1.0d0
      MsT=1.0d0
   end if
   HexT(1,itemp)=Hex0(1)*MsT/Ms
   HexT(2,itemp)=Hex0(2)*MsT/Ms
!   HexT(itemp)=0.5d0*(Hex0(1)+Hex0(2))*MsT/Ms
!   write(6,*) temp0,HexT(itemp)
...
magR=magR1*muB ! gr*J*muB ! for each site
magRT=magR1*muB ! for each site
...
! Interaction
JRFe=12.0d0*d*muB/Zrf*gf*(gr/(gr-1.0d0))*nrf ! d*muB (kOe*muB=Merg) 
JFeFe=6.0d0*d*muB/Zff*gf**2*nff
JRR=0.0d0
