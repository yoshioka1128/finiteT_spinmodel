subroutine multi3j(L1,S1,J1,ZUst,imax,stfct,nJex,n4f,rdscn,HexT,nr,nrmax,lambda,d3j,t3j,eigen,g,JM,temp)
implicit none
...
real(8) :: J1, J2, dl, dm, S1, Jin, abc,def,ghi,lambda,dz
...
do ir=1,nr
   ...
   do in=2,6,2
      abc=4.0d0*HexT(ir)/(lambda*(J1+1.0d0))*rdsjjp(1)*(-rdcjjp(in,1))
      dz=0.0d0
      do ii=1,nJ1
...
do ir=1,nr
   do in=2,6,2
      dl=dble(in)
      abc=(2.0d0*HexT(ir)/(lambda*(J1+1.0d0)))**2*2.0d0*(-rdsjjp(1)*rdcjjp(in,1))
      rdscn(in,ir,1)=abc*rdsjj(1)
      rdscn(in,ir,2)=abc*rdsjj(2)

      abc=(2.0d0*HexT(ir)/(lambda*(J1+1.0d0)))**2*(-rdsjjp(1)**2)
      rdscn(in,ir,3)=abc*rdcjj(in,1)
      rdscn(in,ir,4)=abc*rdcjj(in,2)
   end do ! do in
end do ! do ir
...
do ir=1,nr
   ...
   do in=2,6,2
      ...
      if(temp.eq.500.0d0) stop
      if(temp.ne.0.0d0) then
         t3j(in,ir)=t3j(in,ir)/dz
      end if
!      write(6,*) "t3j",t3j(in,ir)
!      write(6,*) "t3j",t3j(in,ir),abc*def,dexp(-eigen(ii)/temp)
   end do ! do in
end do ! do ir
