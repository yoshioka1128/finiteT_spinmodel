subroutine CF_Ki_interface(ax,cx,Hex,temp0,Klcr,Klcfe,nr,nfe,nunit,irsub,KuFe,HexT)
implicit none
...
real(8) :: K11,K22,K1,K2,K3,K4,K5,Kloc(9,16,0:700),Klocave(9,0:700),fct(16)
...
real(8) :: aleng,bleng,cleng,volume,angle,K1til(0:2),K2til(0:2),temp0,K1tilloc(0:2,16),K2tilloc(0:2,16),Klcr(9,nr*nunit),Klcfe(9,nfe*nunit),KuFe
...
!if(rare.eq."Nd") then
! inddy=1
   open(34,file="~/CFPs/cfpNdinterface_iii_Rmt2.8.txt")
...
!   write(6,*) ach,bch,cch,dch
   ...
   do i=1,iLMmax
      read(34,*) l,m,abc
      if(mod(l,2).eq.0.and.mod(m,2).eq.0.and.l.gt.0.and.iatnum.ge.9.and.iatnum.le.24) then
         write(6,*) l,m,abc
         ! middle layer
         if(iatnum.eq.9)  Alm(l,m,13)=sym(m)*abc
         if(iatnum.eq.10) Alm(l,m,14)=sym(m)*abc
         if(iatnum.eq.11) Alm(l,m,15)=sym(m)*abc
         if(iatnum.eq.12) Alm(l,m,8)=sym(m)*abc
         if(iatnum.eq.13) Alm(l,m,5)=sym(m)*abc
         if(iatnum.eq.14) Alm(l,m,6)=sym(m)*abc
         if(iatnum.eq.15) Alm(l,m,7)=sym(m)*abc
         if(iatnum.eq.16) Alm(l,m,16)=sym(m)*abc
         ! top layer
         if(iatnum.eq.17) Alm(l,m,11)=sym(m)*abc
         if(iatnum.eq.18) Alm(l,m,12)=sym(m)*abc
         if(iatnum.eq.19) Alm(l,m,10)=sym(m)*abc
         if(iatnum.eq.20) Alm(l,m,1)=sym(m)*abc
         if(iatnum.eq.21) Alm(l,m,3)=sym(m)*abc
         if(iatnum.eq.22) Alm(l,m,4)=sym(m)*abc
         if(iatnum.eq.23) Alm(l,m,2)=sym(m)*abc
         if(iatnum.eq.24) Alm(l,m,9)=sym(m)*abc
      end if
   end do
end do
180 write(6,*) "stop"
...
end subroutine CF_Ki_interface
